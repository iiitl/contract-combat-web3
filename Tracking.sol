...
contract Tracking {
    ...
    function createShipment(address _receiver, uint256 _pickupTime, uint256 _distance, uint256 _price, uint256 _estimatedDeliveryTime) public payable {
        require(msg.value == _price, "Payment amount must match the price");
        bytes32 shipmentId = keccak256(abi.encodePacked(msg.sender,_receiver,_pickupTime,shipmentCount,block.timestamp
         )
        );
    ...
    function startShipment(address _sender, address _receiver, uint256 _index) public payable{
        Shipment storage shipment = shipments[_sender][_index];

        require(shipment.receiver == _receiver, "Invalid receiver.");
        require(shipment.status == Status.STATUS_PENDING, "Shipment is not pending");
        require(
    ...
    function completeShipment(address _sender, address _receiver, uint256 _index) public {
        ...
        require(shipment.receiver == _receiver, "Invalid receiver.");
        require(shipment.status == Status.STATUS_IN_TRANSIT, "Shipment is not in transit");
    ...
    function getShipment(address _sender, uint256 _index) public view returns (
        address, address, uint256, uint256, uint256, uint256, uint256, bool,bytes32
     ) {
    ...
    function updateLocation(bytes32 _shipmentId, string memory _location) public{
    ...
    function getLocation(bytes32 _shipmentId) public view returns (string memory) {
